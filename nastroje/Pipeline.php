...
class Pipeline {
    private $cil = __DIR__ . '/../vystupy/postavy.pdf';
    private $jazyk = 'en-gb';
    // jazyk pro dělení slov
    private $slozkaTextu;
    private $slozkaSablon = __DIR__ . '/../sablony';
    private $slozkaVystupu = __DIR__ . '/../vystupy';
    private $wwwRoot = '../..'; // relativní cesta od vygenerovaných html k rootu

    public function __construct($slozkaTextu, $jazyk) {
    ...
    public function vytvoritPostavy() {
        ...
        if (!$nahled) {
            if (!is_dir(dirname($cil))) {
                mkdir(dirname($cil));
                if (PHP_SAPI !== 'cli') {
                    chmod(dirname($cil), 0777);
                } // zpřístupnit všem, pokud zavoláno přes web
            }
    ...
    public function vytvoritZpravu() {
    ...
    private function assertDir($dir) {
        $parent = dirname($dir);
        if (!is_dir($parent)) {
            $this->assertDir($parent);
        } // rekurzivně vytvořit předky, pokud je to potřeba
        if (!is_dir($dir)) {
            mkdir($dir);
        }
        if (!is_dir($dir)) {
            throw new Exception('nepodařilo se vytvořit složku');
        }
    }
    ...
    private function pruh($scena, $postava) {
        foreach ([
            ...
            if (is_file($this->slozkaSablon . $cesta)) {
                return $this->wwwRoot . '/sablony' . $cesta;
            }
        }
